 ...
 */
public class SEED_PTS implements Ordering {
    private List<Order> orderList;
    private List<Path> pathList;
    private int maxCapa;
    private int numAisle;

    public SEED_PTS(List<Order> orderList, List<Path> pathList, int maxCapa, int numAisle) {
    ...
    public Result getResult() {
        ...
        while (orderList_.size() != 0) {
            if (newBatch.getOrderList().size() == 0) {
                ...
                if (newBatch.getCapa() + orderPicked.getCapa() <= maxCapa) {
        ...
        return new Result(Type.SEED_PTS, batchList, executeTime);
    }
    ...
    private List<Order> extractOrders(Batch batch, Order order) {
